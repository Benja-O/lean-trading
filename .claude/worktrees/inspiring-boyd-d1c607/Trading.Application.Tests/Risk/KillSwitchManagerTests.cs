    ...
    public class KillSwitchManagerTests
    {
            ...
            TimeSpan? coolingOffPeriod = null)
        {
            return new KillSwitchManager(
                _portfolioState, _orderRouter, _clock, _logger, _eventBus,
        ...
        [Fact]
        public void ActivateKillSwitch_PublishesRiskLimitBreachedEvent()
        {
            var capturedEvents = new CapturingEventSubscriber<RiskLimitBreachedEvent>(_eventBus);
            var killSwitch = BuildKillSwitchManager();

            killSwitch.ActivateKillSwitch(RiskLimitBreachReason.Manual, "test reason");

            Assert.Single(capturedEvents.CapturedEvents);
            Assert.Equal(RiskLimitBreachReason.Manual, capturedEvents.CapturedEvents[0].Reason);
            Assert.Equal("test reason", capturedEvents.CapturedEvents[0].Description);
        }
    }
}
