...
- **Track de estrategias:**
  ...
  - **Momentum residual / beta-neutral (universo ancho):** ❌ NO-GO Capa A (2026-06-27). Long-only residual mejora vs crudo (5/8 IS≥0.5, meseta SI) PERO el Sharpe IS es ~enteramente beta de mercado (cartera long de alts β≈1.2 en bull): al neutralizar la beta (hedge short BTC) el edge desaparece (2/8 positivas, media negativa, GATE NEUTRAL FAIL); OOS colapsa 0/8 identico al crudo (fuga de beta / sin momentum idiosincratico persistente). **Cierra toda la familia momentum.** Ver strategy_experiments.md.
- **Arquitectura de cartera (Parte I):** decidida en lo conceptual. Pendientes de bajar a números: I.7. **Aviso:** con S1 ❌, eje vol ❌ y momentum (crudo+residual) ❌, los tres ejes de la Parte I (trend/carry/reversión) siguen vacíos. El hueco de mecanismo neutral sigue abierto.
- **Inflexion estructural (2026-06-27):** quedan descartados por Capa A **todos los ejes accesibles probados** — carry (Fase 0, costos), reversion microestructura ×4 (costos), trend majors + gateado HMM (sin edge), vol ×3 (bloqueo de venue), momentum crudo + residual/beta-neutral (el IS era beta, no alfa). El patron transversal es una **restriccion estructural**: edge accesible + costos retail (taker) + datos accesibles sobre Binance perps. **Pendiente: reevaluacion estructural** — cambiar un supuesto del marco (ejecucion maker/menor costo; venue con opciones para el carril de vol; otra clase de dato) vs. aceptar los limites del marco actual. Decision de research-direction/usuario.

**Próximo paso:** Reevaluacion estructural (pendiente de decision). No quedan hipotesis vivas dentro del marco actual (Binance perps, costos taker retail, datos accesibles).
