...
| ⬜ | OPS-2 | `StrategyHealthMonitor` — implementación runtime de `POLICY.md` | Hito C | Componente nuevo en `Trading.Application` que implementa `IRiskMonitor` (refactor #4) y consume `OrderFilledEvent` del bus para mantener métricas en vivo por estrategia (drawdown desde ATH, drawdown rolling 30 días, profit factor rolling 30 trades, expectancy rolling 30 trades). Compara contra los umbrales **U1-U4 definidos en `POLICY.md` sección 3** y dispara `RiskLimitBreachedEvent` con `RiskLimitBreachReason.StrategyDegradation` cuando se cruzan. U3 y U4 solo se evalúan tras 50 trades acumulados (ver POLICY 3.4). Acción al disparar: liquidación inmediata de la posición de la estrategia + pausa. Se registra en el array de monitors de `RiskOrchestrator`, sin tocar nada existente (open-closed). La automatización es necesaria porque la inspección humana de métricas no escala y falla bajo estrés operativo. |
| ⬜ | DEUDA-1 | Diagnóstico del test `AccordHmmClassifierReferenceTests` actualmente skipeado | Hito C | Documentado en ADR-020. Test marcado `[Fact(Skip = "...")]` por convergencia degenerada de Baum-Welch sobre serie sintética con K=3. Modelo de producción (BTCUSDT, K=4) NO presenta el síntoma. Diagnóstico antes de paper trading. |
| ⬜ | DEUDA-2 | `TradingAlgorithmHost.Initialize()` se ejecuta dos veces en backtest | Hito C (validar si aplica también en live) | Descubierta al cerrar INFRA-2 Pieza C. Lean invoca `Initialize()` dos veces en backtest (presumiblemente warm-up + run real). Efectos: doble suscripción al `IDomainEventBus`, doble instanciación de `HttpClient` y del sink de logs. Idempotente en efecto observable; sin impacto funcional sobre métricas. Fix esperado: guard de idempotencia al inicio de `Initialize()`. Validar primero si afecta también a paper trading (Hito C); si en live `Initialize()` se invoca una sola vez, la deuda es solo de backtest y puede dejarse a más largo plazo. |
| ⬜ | DEUDA-3 | Logs durante `Initialize()` tienen timestamp del epoch de QC (1997-12-31) | No bloquea Hito C | Descubierta al cerrar INFRA-2 Pieza C. `LeanClock.UtcNow` devuelve `_algorithm.Time`; antes de `SetStartDate()`, ese valor está en el epoch del motor de QC. Los primeros logs del JSONL y el `ProcessStartedUtc` del heartbeat muestran `1997-12-31T19:00:00`. Cosmético, no funcional. No aplica en paper/live (sin `SetStartDate`). Fix opcional: fallback al wall clock cuando `_algorithm.Time` sea anterior a un threshold razonable; requiere ADR propio cuando se aborde. |
