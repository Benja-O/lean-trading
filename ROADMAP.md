...
#### 🔄 RECORDER-1 — Feed del recorder vía REST polling + escala por IP de egreso
**Bloque:** data plane (continuación de HITO-D-feat / ADR-048).
**Estado:** implementado 2026-06-19 (`ITradeFeed` + `BinanceAggTradeRestFeed`, REST default) y **gap-fill de arranque desde Vision con puente al vivo** (`StartupSeeder`): el recorder arranca con historia inmediata, sin esperar días, y sin paso manual de siembra. 7 tests nuevos + smoke tests reales OK (REST live, y Vision→puente→REST contiguo con CVD continuo). **Pendiente:** re-deploy del recorder al VPS con `RECORDER_FEED=rest` y validar barras cerrándose en el store.
...
| 🔄 | HITO-D | Live trading con capital chico | Hito D-prev ✅ + estrategia walk-forward ✅ + HITO-D-feat ✅ | **Infra validada en VPS (2026-06-17). HITO-D-feat completado 2026-06-17** — pipeline live de features listo. Pendiente: (1) deploy del nuevo build al VPS (incluye cambios en `Brokerages.Binance.dll` por captura de `is_buyer_maker`); (2) arrancar `LeanLive` (NSSM); (3) verificar logs: primer tick recibido, primera barra 1h cerrada, `ComputeAndAdd` exitoso, primera señal microestructural generada. Items de infra previos: reduceOnly (ADR-044), minimal-position-mode (ADR-045), test supervisado SL/TP validado. |
