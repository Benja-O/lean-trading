...
| 🔄 | HITO-D | Live trading con capital mínimo | Hito D-prev ✅ + estrategia walk-forward ✅ + HITO-D-feat ✅ | **Infra validada en VPS (2026-06-17). HITO-D-feat completado 2026-06-17** — pipeline live de features listo. **Decisión (ADR-050, 2026-06-20):** las 2 estrategias de microestructura van **directo a live con minimal-position-mode permanente** (validación operativa con fills reales en vez de paper; capital despreciable). LeanLive **nunca corrió con estas estrategias** — hay que implementarlo. Pendiente: (1) clock sync (ADR-043); (2) deploy del build con DLLs del fork (`QuantConnect.Common.dll` + `QuantConnect.Brokerages.Binance.dll`, ADR-044/045) a `C:\Lean\Live\`; (3) `config.json` environment `live-futures-binance`, API key con Futures + IP whitelist, cuenta One-way; (4) `strategies.json` con las 2 microestructura + `minimal-position-mode: true`; (5) env vars NSSM (`MICROSTRUCTURE_STORE_DIR=C:\Lean\MicrostructureStore`, `HEALTHCHECKS_PING_URL`); (6) `StrategyHealthMonitor` real (U1-U4) activo; (7) verificar logs: sin -1021/-2015/-4061, warmup cargando store, primera barra 1h, `ComputeAndAdd`, primera señal, SL/TP con reduceOnly. Caveat (P3, ADR-050): el live-mínimo NO valida slippage a escala. |
