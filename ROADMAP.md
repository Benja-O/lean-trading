...
| 🔄 | HITO-D | Live trading con capital chico | Hito D-prev ✅ + al menos una estrategia con walk-forward aprobado en Hito G ✅ | **Ingeniería completa (2026-06-15), pendiente deploy live por el operador.** Items: (1) SL/TP nativos con reduceOnly validados en vivo (ADR-044); (2) minimal-position-mode + override de min notional (ADR-045); (3) test supervisado SL/TP OK. Deploy = mismo flujo de paper (VPS+NSSM ya documentado en AI.md, corriendo desde 2026-06-03), con delta de config: `live-futures-binance`, `minimal-position-mode: true`, key con Futures trading + IP del VPS en whitelist. Ojo (AI.md): publicar el build completo — el reduceOnly diverge en `QuantConnect.dll` y `Brokerages.Binance.dll`, no solo en `Trading.*`. Requisito **inquebrantable** (walk-forward aprobado) cumplido: 2 candidatas de Hito G. |
