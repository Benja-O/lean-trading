...
| 🔄 | HITO-D | Live trading con capital chico | Hito D-prev ✅ + al menos una estrategia con walk-forward aprobado en Hito G ✅ | **Ingeniería completa (2026-06-15), pendiente provisión de VPS por el operador.** Items: (1) SL/TP nativos con reduceOnly validados en vivo (ADR-044); (2) minimal-position-mode + override de min notional (ADR-045); (3) test supervisado SL/TP OK; (4) runbook + script NSSM de deploy a VPS (`Trading.Research/vps_deployment/`). Falta: provisionar el VPS (recomendado AWS Tokio), correr el runbook, arrancar la corrida continua. Requisito de entrada **inquebrantable** (estrategia walk-forward aprobada) cumplido: 2 candidatas de Hito G. El tamaño "chico" es el riesgo psicológico que el operador puede absorber (POLICY P4). |
