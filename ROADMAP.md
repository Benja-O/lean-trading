...
| 🔄 | HITO-D | Live trading con capital mínimo | Hito D-prev ✅ + estrategia walk-forward ✅ + HITO-D-feat ✅ | **Infra validada en VPS (2026-06-17). HITO-D-feat completado 2026-06-17** — pipeline live de features listo. **Decisión (ADR-050, 2026-06-20):** las 2 estrategias de microestructura van **directo a live con minimal-position-mode permanente** (validación operativa con fills reales en vez de paper; capital despreciable). LeanLive **nunca corrió con estas estrategias** — hay que implementarlo. **Prep dev-side hecho (2026-06-20):** build+test verde (282 tests) y `dotnet publish -c Release -r win-x64 --self-contained` a `publish\live\` (incluye los DLLs divergentes del fork `QuantConnect.Common.dll` + `QuantConnect.Brokerages.Binance.dll`, ADR-044/045) + `Launcher/config.live.template.json` versionado (environment `live-futures-binance`, `minimal-position-mode` top-level, placeholders de keys); `strategies.json` ya trae solo las 2 microestructura × BTC/ETH/SOL 1h. **Verificado en código:** el `StrategyHealthMonitor` real se cablea incondicional en el host (no Null) y `minimal-position-mode` se lee de **config.json** (`Config.GetBool`, gateado por LiveMode), **no de strategies.json**. Pendiente (VPS): (1) clock sync (ADR-043); (2) copiar `publish\live\` → `C:\Lean\Live\`; (3) `config.json` desde el template con API key Futures + IP whitelist, cuenta One-way; (4) **`minimal-position-mode: true` en config.json** (NO en strategies.json: ahí se ignora y el sizer vuelve a risk%); (5) env vars NSSM (`MICROSTRUCTURE_STORE_DIR=C:\Lean\MicrostructureStore`, `HEALTHCHECKS_PING_URL` con UUID propio); (6) `StrategyHealthMonitor` real (U1-U4) — ya por defecto; (7) verificar logs: sin -1021/-2015/-4061, warmup cargando store, primera barra 1h, `ComputeAndAdd`, primera señal, SL/TP con reduceOnly. Caveat (P3, ADR-050): el live-mínimo NO valida slippage a escala. |
