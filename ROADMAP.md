...
| ⬜ | HITO-D-feat | Pipeline de features de microestructura en vivo | Infra de Hito D ✅ | **Bloqueante de Hito D — descubierto 2026-06-17 en el deploy live.** Las estrategias aprobadas (TradeSize/Cvd) leen features de un CSV histórico estático (`MicrostructureRegistry`: `Load()` una vez en Initialize, inmutable; `GetBar(ts)` exacto por timestamp). En vivo, la barra 1h actual no está en el CSV → `null` → sin señal. Nunca corrieron forward (IS/OOS de Hito G fue todo sobre el CSV; paper corría EmaCross que no usa features). Falta: suscribir el stream de **aggTrades** de Binance y computar las 8 features (ofi, cvd, cvd_delta, arrival_rate, mean_trade_size, buy_sell_ratio, price_return) por barra 1h en tiempo real, alimentando al `IMicrostructureProvider`. Sin esto las estrategias aprobadas no tradean live. Diseño pendiente. |
| 🔄 | HITO-D | Live trading con capital chico | Hito D-prev ✅ + estrategia walk-forward ✅ + **HITO-D-feat** (pipeline live) | **Infra validada en VPS (2026-06-17): corre, balance OK, reduceOnly validado (ADR-044), minimal-position-mode (ADR-045), servicio `LeanLive`/NSSM. PERO live PAUSADO** — las estrategias aprobadas no señalizan en vivo por falta del pipeline de features (HITO-D-feat). Items de infra hechos: (1) reduceOnly; (2) minimal-position-mode; (3) test supervisado SL/TP. Deploy = flujo de paper + delta de config (`live-futures-binance`, `minimal-position-mode: true`, whitelist IP del VPS); publicar build **completo** (reduceOnly diverge en `QuantConnect.Common.dll` y `Brokerages.Binance.dll`). `LeanLive` detenido (arranque manual) hasta tener el pipeline. |
